...
if result:
	...
	if env_template in instance["name"]:
	    # group by name splited by "-" and add all ip hosts with the same name to one group
	    #print(instance["name"].split("-")[0])
	    if instance["name"].split("-")[0] in instance["name"]:
		if (inventory.get(instance["name"].split("-")[0])):
		    inventory.get("docker").get("hosts").append(instance["networkInterfaces"][0]["primaryV4Address"]["oneToOneNat"]["address"])
		else:
		    host = {instance["name"].split("-")[0]:{"hosts": [instance["networkInterfaces"][0]["primaryV4Address"]["oneToOneNat"]["address"]]}}
